...
SELECT
  ...
  CASE
    ...
    THEN ROUND(COUNT(p.id) FILTER (WHERE p.is_correct = true)::NUMERIC / COUNT(p.id) FILTER (WHERE p.is_correct IS NOT NULL) * 100, 1)
    ELSE 0
  END AS accuracy
FROM users u
...
BEGIN
  IF NEW.winner IS NOT NULL AND OLD.winner IS DISTINCT FROM NEW.winner THEN
    UPDATE picks SET
      ...
      payout = CASE WHEN pick = NEW.winner THEN wager ELSE -wager END
    WHERE game_id = NEW.id;
    UPDATE users u SET coins = coins + p.payout
    FROM picks p WHERE p.game_id = NEW.id AND p.user_id = u.id AND p.payout IS NOT NULL;
    INSERT INTO coin_transactions (user_id, amount, reason)
    SELECT p.user_id, p.payout,
      CASE WHEN p.is_correct THEN 'Won pick: ' || NEW.home_team || ' vs ' || NEW.away_team
           ELSE 'Lost pick: ' || NEW.home_team || ' vs ' || NEW.away_team END
    FROM picks p WHERE p.game_id = NEW.id AND p.payout IS NOT NULL;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER grade_picks_on_result AFTER UPDATE OF winner ON games FOR EACH ROW EXECUTE FUNCTION grade_picks_for_game();

CREATE OR REPLACE FUNCTION award_weekly_coins()
RETURNS void AS $$
BEGIN
  UPDATE users SET coins = coins + 15 WHERE status = 'approved' AND is_admin = false;
  INSERT INTO coin_transactions (user_id, amount, reason)
  SELECT id, 15, 'Weekly coin bonus' FROM users WHERE status = 'approved' AND is_admin = false;
END;
$$ LANGUAGE plpgsql;

INSERT INTO users (username, status, is_admin, coins) VALUES ('zay', 'approved', true, 0);
