  ...
  return (
    <div>
      ...
      {myRank >= 0 && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 mb-6 flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <span className="text-amber-500 text-sm ml-2">{entries?.[myRank]?.coins ?? 0} 🪙</span>
          </div>
        </div>
      )}
      ...
      <div className="card overflow-hidden">
        ...
        {(entries ?? []).length === 0 ? (
          <div className="py-16 text-center text-slate-500 text-sm">No picks made yet. Be first!</div>
        ) : (
          entries!.map((entry, i) => {
            ...
            return (
              <div key={entry.id} className={`grid grid-cols-12 gap-2 px-5 py-4 border-b border-slate-800/40 ${isMe ? 'bg-amber-500/5' : 'hover:bg-slate-800/20'}`}>
                ...
                <div className="col-span-3 text-center font-syne font-bold text-amber-400">
                  {Number(entry.coins)}
                </div>
                <div className="col-span-2 text-center text-green-400 font-semibold text-sm">
                  {Number(entry.correct_picks)}
                </div>
